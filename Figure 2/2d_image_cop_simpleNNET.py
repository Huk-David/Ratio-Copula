...
for epoch in (range(100000)):
    ...
    r_q = ratio(tt(np.random.randn(100000,2)).float())
    #loss = (-(r_p /(1+r_p)).log() - (1/(1+r_q)).log() ).mean()
    loss = loss_nce(r_p, r_q, 10000, 100000)
    loss.backward()
    optimizer.step()
    if epoch % 100 == 0:
        with torch.no_grad():
...
model_path = f'model_parameters_W_ratio_Image/NNet_ratio_cop_simpleNNet_1Ms_100Ke_0_05_noise_100Kbatches.pt'
