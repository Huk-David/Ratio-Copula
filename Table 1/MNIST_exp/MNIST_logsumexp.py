...
# the one below was unstable and causing overflow
    ...
    for epoch in (range(num_epochs)):
        ...
        if (epoch+1) % 10 == 0:
            ...
            if not np.isnan(running_loss):
                torch.save(model_GG_CNN.state_dict(), f'GGCNN_{num_epochs}epoch_mnist_seed_{seed}.pth')
            else:
                break
    # save the model
    filename = f'logsumexpGGCNN_{num_epochs}epoch_mnist_seed_{seed}.pth'
    ...
    # save the cov
    GG_cov = np.cov(X_train.reshape(-1,28*28).T)
    np.save(f'logsumexpGGCNN_500epoch_mnist_seed_{seed}_cov.npy', GG_cov)
    ...
    # Save the samples 
    np.save(f'logsumexpGGCNN_HM100burnin_N01_{seed}_500epoch_samples_mnist.npy', sample_GG_CNN)
